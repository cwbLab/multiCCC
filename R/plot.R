...
#'
#' @description
#' Plot a dot plot for the results of the categorical variable.
#'
...
#' @param fill The column used for color fill. Options are p or p.adj.
#' @param threshold The threshold for p or p.adj. Corresponds to the fill parameter.
...
#'
#' #4.plot_dot
#' plot_dot( res = filter.binary$binary.res ,
#'           ligand = c("S100A8" , "S100A8" , "CD47") ,
#'           threshold =  1 ,strip.text = 8 )

#' plot_dot( res = filter.anova$anova.res ,
#'           ligand =  c("HP" ,"S100A9" , "S100A8" ),
#'           threshold =  1 ,strip.text = 8 )

plot_dot <- function( res , data.used = 'filtered.result',fill = 'p.adj', threshold =  0.05 , colors =  c('#0000FF', "#F7F7F7", '#FF0000'),
  ...
  p <- ggplot(plot_data, aes(x = Group, y = LR )) +
    ...
    theme(
      ...
      axis.text.x = element_text(  size = x.text ,  angle = 90, vjust = 1 ,hjust = 1),
...
#' Line plot
#' @description
#' Plot a fitted curve for the continuous variable.
#'
#' @param res The object returned by the filterCCC function.
...
#'
#' #4.plot_line
#' CCC.ID <- 'cell2_cell3.CALR_ITGAV'
#' plot_line( res = filter.glm$glm.res,
#'            CCC.ID = CCC.ID , p.adj = F )
#'
plot_line <- function( res, CCC.ID, p = T, p.adj =T,
